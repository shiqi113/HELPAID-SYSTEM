...
public class DonorContributionDialog extends JDialog {
	...
	public static void main(String[] args) {
		try {
			DonorContributionDialog dialog = new DonorContributionDialog
											(null,null);
	 ...
	 * Create the dialog.
	 */
	public DonorContributionDialog(HELPAid helpAid,Appeal appeal) {
		...
		contentPanel.setLayout(new GridLayout(0, 2, 0, 0));
		{
			...
			{
				table = new JTable();
				table.setModel(allcurrentappealTM);
		...
		}
		{
			...
			getContentPane().add(buttonPane, BorderLayout.SOUTH);
			{
				...
				submitButton.addActionListener(new ActionListener() {
					public void actionPerformed(ActionEvent e) {
						...
						Appeal foundAppeal = helpAid.findAppeal(appealID);
						if(appealID.isEmpty())
						{
			            	JOptionPane.showMessageDialog(
			            			 DonorContributionDialog.this, 
			            			"Input Cannot Empty");
						} else {
							if (foundAppeal == null) {
				            	JOptionPane.showMessageDialog(
				            	...
				            	if(cashRB.isSelected()) {
				            		try {
										...
										if(amount < 0) {
											...
											Contribution contribution = new CashDonation
													(foundAppeal,amount, channel, ref);
											foundAppeal.addContribution(contribution);
											JOptionPane.showMessageDialog(
							            			 DonorContributionDialog.this, 
							            			 contribution);
											DonorContributionDialog.
														this.dispose();
										}
			...
			}
			{
				...
				cancelButton.addActionListener(new ActionListener() {
					public void actionPerformed(ActionEvent e) {
						DonorContributionDialog.this.dispose();
					}
				});
